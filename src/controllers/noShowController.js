...
const reportUser = asyncHandler(async (req, res) => {
    ...
    if (!request) return res.status(404).json({ message: "no such exchangeRequest exists" })

    if (request.status !== "accepted") return res.status(400).json({ message: "reciever should accept ur request before u report him " })
    ...
    const alreadyReported = await noShowReportModel.findOne({ exchange: requestID, reportedBy })
    if (alreadyReported) return res.status(400).json({ message: "already reported" })

    await noShowReportModel.create({ exchange: requestID, reportedBy, reportedUser })
